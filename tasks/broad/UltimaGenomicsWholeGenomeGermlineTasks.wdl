...
task SamToFastqAndBwaMemAndMba {
  ...
  command <<<
    ...
    # if ref_alt has data in it, or allow_empty_ref_alt is true for references without BWA ALT contigs
    if [ -s ~{alignment_references.ref_alt} ] || ~{allow_empty_ref_alt}; then
    ...
    # else ref_alt is empty or could not be found, and allow_empty_ref_alt is false
    else
